...
3. Grab the *Client ID* and *Client Secret* after registering the application
   ...
   .. code-block:: console
       $ export CILOGON_APP_CREDENTIALS_KEY=my_github_client_id
       $ export CILOGON_APP_CREDENTIALS_SECRET=my_github_client_secret
...
4. Create database and tables:
   ...
   .. code-block:: console
       ...
       $ pip install -e .[all]
       $ cd examples
       $ export FLASK_APP=cilogon_app.py
       $ ./app-setup.sh

You can find the database in `examples/cilogon_app.db`.
...
CILOGON_APP_CREDENTIALS = dict(
    consumer_key=os.environ.get("CILOGON_APP_CREDENTIALS_KEY"),
    consumer_secret=os.environ.get("CILOGON_APP_CREDENTIALS_SECRET"),
)
...
app.config.update(
    SQLALCHEMY_DATABASE_URI=os.environ.get(
        "SQLALCHEMY_DATABASE_URI", "sqlite:///cilogon_app.db"
    ),
    OAUTHCLIENT_REMOTE_APPS=dict(
        cilogon=cilogon.REMOTE_APP,
    ),
    CILOGON_APP_CREDENTIALS=CILOGON_APP_CREDENTIALS,
    DEBUG=True,
    SECRET_KEY="TEST",
...
def cilogon():
    ...
    if not current_user.is_authenticated:
        return redirect(url_for("invenio_oauthclient.login", remote_app="cilogon"))
